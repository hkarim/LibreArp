...
void LibreArp::processMidi(int numSamples, juce::MidiBuffer &midi) {
  ...
  if (!*bypass && cpi.isPlaying && !this->events.events.empty() && this->events.loopLength > 0) {
    /*
    std::cout
      << "timebase = " << this->events.timebase
      << ", sampleRate = " << getSampleRate()
      << ", cpi.bpm = " << cpi.bpm
      << ", cpi.timeSigNumerator = " << cpi.timeSigNumerator
      << ", cpi.timeSigDenominator = " << cpi.timeSigDenominator
      << ", cpi.timeInSeconds = " << cpi.timeInSeconds
      << ", cpi.ppqPosition = " << cpi.ppqPosition
      << ", cpi.ppqLoopStart = " << cpi.ppqLoopStart
      << ", cpi.ppqLoopEnd = " << cpi.ppqLoopEnd
      << std::endl;
    */
...
juce::ValueTree LibreArp::toValueTree() {
  ...
  tree.setProperty(TREEID_NON_PLAYING_MODE_OVERRIDE, NonPlayingMode::toJuceString(this->nonPlayingModeOverride),
                   nullptr);
